...
public class BillingManager implements PurchasesUpdatedListener {
    ...
    public boolean boughtNoAds = false;
        ...
        if (resultCode == BillingResponse.OK) {
            for (Purchase purchase : purchases) {
                if (purchase.getSku().equals("no_ads")) {
                    DBMain dbMain = (DBMain) mActivity;
                    Toast.makeText(mActivity, "You've purchased No Ads! Restart App to instigate changes", Toast.LENGTH_LONG).show();
                    dbMain.playingAd = true;
                    boughtNoAds = true;
                    DBMenuSystem.doConfirmQuit(dbMain);
                    return;
                }
            }
